...
namespace Microsoft.Azure.Devices.Applications.RemoteMonitoring.UnitTests.Web.WebApiControllers
{
    public class TelemetryApiControllerTests : IDisposable
    {
        ...
        private readonly Mock<ILocationJerkLogic> locationJerkLogic;
        ...
        public async void GetLatestAlertHistoryAsyncTest()
        {
            ...
            Assert.Equal(data.JerkLocations.Count, itemModels.Count());
            Assert.Equal(data.TotalAlertCount, itemModels.Count());
            Assert.Equal(data.TotalFilteredCount, itemModels.Count());
            Assert.Equal(data.JerkLocations.Select(d => d.Status == LocationStatus.Caution).Count(), itemModels.Count());
            ...
            Assert.Equal(data.JerkLocations.Select(d => d.Status == LocationStatus.Critical).Count(),
                itemModels.Count());
            ...
            res.AssertOnError();
            data = res.ExtractContentAs<AlertHistoryResultsModel>();
            Assert.Equal(data.JerkLocations.Select(d => d.Status == LocationStatus.Caution).Count(),
                itemModels.Count());
        }
