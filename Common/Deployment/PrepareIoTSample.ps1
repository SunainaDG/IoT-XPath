...
$locationRulesFilePath = "$(Split-Path $MyInvocation.MyCommand.Path)\locationrules.json"
...
{
    $legacyNameExists = (Find-AzureRmResourceGroup -Tag @{"IotSuiteType" = $suiteType} | ?{$_.ResourceGroupName -eq "IoTXPath"}) -ne $null
    if ($legacyNameExists)
    {
        $suiteName = "IoTXPath"
    }
...
# Setup default location rules blob for stream analytics
UploadFile $locationRulesFilePath $storageAccount.StorageAccountName $resourceGroupName "locationrules" -secure $true
...
if ($cloudDeploy)
{
    ...
    ## here we must add logic for google map key prompt
    ####################
    ##################
...
UpdateEnvSetting "FeedbackEventHubName" $result.Outputs['ehFeedbackName'].Value
UpdateEnvSetting "FeedbackEventHubConnectionString" $result.Outputs['ehFeedbackConnectionString'].Value
if($cloudDeploy)
{
    UpdateEnvSetting "MapApiQueryKey" $result.Outputs['mapsQueryKey'].Value
}
